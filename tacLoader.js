  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on("data", (row) => {
        ...
        tacMap.set(tac, parseTacEntry(brand, specs));
        count++;
      })
      .on("end", () => {
...
function parseTacEntry(brand, specs) {
  let modelName = "";
  let modelCode = "";
  ...
  return { brand, modelName, modelCode, fullSpecs: specs };
}
...
// Add/update a single TAC entry (untuk import admin)
function addTacEntry(tac, brand, specs) {
  if (!tac || tac.length < 8) return;
  tacMap.set(tac, parseTacEntry(brand.toUpperCase(), specs));
}

module.exports = { loadTacDatabase, lookupTac, getTacCount, addTacEntry };
